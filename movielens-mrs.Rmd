...
To remain consistent with the earlier course artifacts, I have used R version 3.5.2 (2018-12-20) for developing and running my scripts.
...
Before committing to run these artifacts, be aware that the data download (over the internet, in this case) and several operations on that full dataset (including some transformations, plots and model training) take significant time to run. The entire end-to-end script (either when you run model.R in its entirety, or Knit the full movielens-mrs.Rmd) takes about 10 minutes to complete on my Macbook Pro laptop.
...
The goal of this project is to develop a well-performing movie recommendation system. The main objective of such a system is to use collected/generated data (the 10M subset of MovieLens dataset, in this case) to train a model that will then predict a list of top movies that a given user may like to watch.

In addition to developing the machine learning models, data is analyzed, explored and visualized in order gain several insights. A copy of the dataset was then further curated to aid in further visualizations, and to more conveniently exploit (in the future) the insights learned from this project.
...
The basic underlying strategy boils down to filling in the ratings for the unfilled slots of all the movie/user combinations, based on the small subset of previously filled spots (ratings by actual users). 

The end goal is then to use this pattern/model i.e. use the slots filled during training, to filter the best rated movies for a user and recommend them to her.

The metric, or performance used here is RMSE (the root mean squared error, predicted on the formally allocated validation dataset).


## Process, and Model Selection
The training approach or strategy is to not use linear regression due to the large size of the dataset and its combinatorial vastness.  Instead the approach taken (using mean rating and “effects”) is developed to practically achieve the same effect as linear regression would.

I start with a random baseline model where the model always gives an average rating.  I then add movie effects to my model. This is followed by adding the user effect. Then I add regularization. Regularization allows me to through out the outlier ratings in order to prevent overtraiing or overfitting.

This incremental approach will also help me understand the contribution of each of the two effects or biases, as well as that of regularization, including the specific hyperparameters tried (lambda in this case) to achieve best results.

Prior to the modeling exercise I explored and visualized the data that I later use in training. The hope is that this allows me and the reader to get an intuition of the independent variables (including the movie itself, the user desiring a rating, the time a rating is given, and the genre(s) the movei belongs to) and their effects on the final rating a movie receives from a user. This exploration also brings some insights on what other independent effects we have that can potentially further improve the RMSE and thus provide even more valuable movie suggestions. We need to be cognizant though about which parameters to use, and which we simply ignore, in order to avoid overtraining the model and to not unnecessarily slow its training (and potentially even the prediction-time) performance. 
...
### Statistics related to movies and users in the dataset
...
edx %>% group_by(movieId) %>%
  ...
  ggtitle("Distribution of Total Ratings Received for Every Movie") +
...
edx %>% group_by(userId) %>%
  ...
  ggplot(aes(x=average)) + 
  geom_histogram(color="black", fill="orange") +
  ggtitle("Distribution of Total Ratings Given by Every User") +
...
edx %>%
  ...
  ggtitle("Ratings Average Across the Data Collection Window") + 
  ylab("Average rating during the week") + 
  xlab("Time in weekly slices")
```
...
edx_x %>% 
  ...
  geom_bar(position="dodge", stat="identity", fill="orange") +
  ggtitle("Ratings Distribution Over Days of the Week") + 
...
## Model 3 - Modeling Movie Effects + User Effects
...
## Model 4 - Modeling Regularization + Movie Effects + User Effects
...
## Model 5 - Modeling Regularization + Movie Effects + User Effects - with picked lambda
...
rmse_results <- bind_rows(rmse_results, tibble(method = "movie and user effects, picked lambda",
...
Performance (computational complexity of the large, cross-connected dataset) prevents me from trying more standardized (out-of-box) models like linear regression, nearest neighbors, SVD (single value decomposition), PCA (principal component analysis), and deep learning. Moving forward, these could be further explored with a reduced version of the training set, or on a cloud platform.

Finally, the runtime (prediction-time) is also an important consideration. We need to ensure that same inputs used in training the model, will also be available (amd available in a usable form) at runtime in a timely, efficient, and relevant manner.
